...
func collectServiceInstallValues(opts serviceInstallOptions, existing map[string]string, in io.Reader, out io.Writer) (map[string]string, error) {
	...
	values["CTR_GO_AUTOMATIONS_DIR"] = strings.TrimSpace(firstNonEmpty(opts.AutomationsDir, existing["CTR_GO_AUTOMATIONS_DIR"], config.DefaultAutomationsDir()))
...
func validateServiceValues(values map[string]string) (map[string]string, error) {
	required := map[string]string{
		...
		"CTR_GO_AUTOMATIONS_DIR":    "--automations-dir",
	...
	checks := []struct {
		...
		{"CTR_GO_AUTOMATIONS_DIR", validateNonEmpty},
