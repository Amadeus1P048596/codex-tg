...
- Omitting the prompt from `/newchat` or `/newthread` starts a title-then-prompt flow. The first plain-text message is the explicit thread title; the second is the first turn prompt. The pending state survives daemon restart, expires after 15 minutes, and can be cleared with `/cancel`.
